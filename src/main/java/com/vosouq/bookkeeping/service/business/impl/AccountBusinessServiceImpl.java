...
//    @Override
//    public Account getOrCreateOnlineUserAccount() {
//        Long userId = userBusinessService.getCurrentUserId();
...
//            account = accountCrudService.createUserRialAccount(userId);
//        }
//        return account;
//    }
...
//        account.decreaseWithdrawableBalance(blockedAmount);
    ...
    public List<AccountTurnoverResponse> getAccountTurnovers(AccountTransactionType transactionType) {
        ...
        return accountAudits.stream().filter(Objects::nonNull)
                .map(audit -> new AccountTurnoverResponse(audit.getTransactionType(), audit.getTransactionAmount().longValue(), audit.getCreateTimestamp()))
